...
modif_copykat <- function(rawmat=rawdata, id.type="S", cell.line="no", ngene.chr=5,LOW.DR=0.05, UP.DR=0.1, win.size=25, norm.cell.names="", KS.cut=0.1, sam.name="", distance="euclidean", n.cores=1){
  start_time <- Sys.time()
  set.seed(1)
    ...
    preN <- norm.cell.names
     ##relative expression using pred.normal cells
      ...
      if (WNS =="unclassified.prediction"){
        Tc <- colnames(rawmat)[which(as.numeric(apply(rawmat[which(rownames(rawmat) %in% c("PTPRC", "LYZ", "PECAM1","TGFBI","CD14","P2RY12","CD163","CD96","CD83","LYN", "CX3CR1")),],2, mean)) >1)]; length(Tc)
